...
namespace GenLibrary.MVVM.Base.EventAttachCommandSmp
{
    /*
    命名空间的引用：
    xmlns:bhv="clr-namespace:GenLibrary.MVVM.Base.EventAttachCommandSmp;assembly=GenLibrary"
    单个事件绑定使用方式如下：
     ...
     在样式中的绑定方式类似如下(其中需要定义带参数的命令)：
     <Style TargetType="{x:Type TextBox}">
            <Setter Property="bhv:CommandBehavior.Event" Value="MouseDoubleClick"/>
            <Setter Property="bhv:CommandBehavior.Command" Value="{Binding SaveRowCommand}"/>
            <Setter Property="bhv:CommandBehavior.CommandParameter" Value="{Binding Text,RelativeSource={RelativeSource Self}}"/>
     </Style>
    */
    /// <summary>
